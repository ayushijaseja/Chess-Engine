...
void generate_pawn_single_push(const Board& B, std::vector<chess::Move>& moveList)
{
    ...
    const uint64_t pawns_to_push = B.white_to_move ? (our_pawns & ~chess::Bitboard::Rank7)
                                                  : (our_pawns & ~chess::Bitboard::Rank2);
...
void generate_push_double_push(const Board& B, std::vector<chess::Move>& moveList)
{
    ...
    const uint64_t pawns_on_start_rank = B.white_to_move ? (our_pawns & chess::Bitboard::Rank2)
                                                        : (our_pawns & chess::Bitboard::Rank7);
...
void generate_pawn_captures(const Board& B, std::vector<chess::Move>& moveList)
{
    ...
    const uint64_t pawns_to_capture = B.white_to_move ? (our_pawns & ~chess::Bitboard::Rank7)
                                                     : (our_pawns & ~chess::Bitboard::Rank2);
...
void generate_pawn_promotion(const Board& B, std::vector<chess::Move>& moveList)
{
    ...
    const uint64_t promoting_pawns = B.white_to_move ? (our_pawns & chess::Bitboard::Rank7)
                                                     : (our_pawns & chess::Bitboard::Rank2);
