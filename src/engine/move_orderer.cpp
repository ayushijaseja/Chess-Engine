...
MoveOrderer::MoveOrderer(const Board& B, int ply, Search& s, bool capturesOnly)
{
    ...
    score_moves(B,ply,s,moveList);

    std::sort(scored_moves.begin(), scored_moves.end(), [](const auto& a, const auto& b) { return a.first > b.first; });
}

void MoveOrderer::score_moves(const Board& B, int ply, Search& s, std::vector<chess::Move>& moveList){
    for(auto& v : moveList)
    {
        int score{};
        if(v.flags() == (chess::FLAG_PROMO) || v.flags() == (chess::FLAG_CAPTURE_PROMO))
        {
